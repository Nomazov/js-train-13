...
function complexConvert(data) {
    ...
    for (const key in parsedJson) {
        ...
        if (typeof parsedJson[key] === "number") {
            newObj[key] = parsedJson[key] + 1;
        } else {
            newObj[key] = parsedJson[key];
        }
    }
...
function searchParamsURL(url) {
    ...
    const searchParams = myUrl.searchParams.entries();
...
function checkURLParameters(url, params) {
    ...
    for (const item of params) {
        result[item] = urlObj.searchParams.has(item);
    };
...
function sortUrlParams(url) {
    ...
    const arr = [...urlObj.searchParams.entries()].sort();
...
function getURLValues(url) {
    ...
    for (const key of keys) {
        const allkeys = searchParams.getAll(key);
        values.push(...allkeys);
    }
